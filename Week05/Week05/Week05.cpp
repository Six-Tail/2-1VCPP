...
// 전역 변수
RECT rectangle = { 0, 0, 0, 0 }; // 사각형의 좌표와 크기를 저장하는 변수
RECT previousRectangle = { 0, 0, 0, 0 }; // 이전 도형의 위치 정보를 저장하는 변수
bool isDrawing = false; // 사각형 그리기 모드 여부
bool isMoving = false;  // 사각형 이동 모드 여부
POINT startPoint; // 그리기 시작 지점
POINT offset; // 이동할 때 사용할 오프셋
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
        if (isDrawing)
        {
            ...
            HDC hdc = GetDC(hWnd);
            HBRUSH pinkBrush = CreateSolidBrush(RGB(255, 182, 193)); // 핑크색
            HPEN blackPen = CreatePen(PS_SOLID, 3, RGB(0, 0, 0)); // 검정색 테두리
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    // 윈도우 생성
    hWnd = CreateWindow(L"MyWindowClass", L"202007026 배지호", WS_OVERLAPPEDWINDOW,
